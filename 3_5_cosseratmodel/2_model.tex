...
First, let us regard the configuration of the soft robot $\mat{g} \in \SE{3}$. Recall that the spatial evolution of the curve is described by $\p \mat{g}/\p\sigma = \mat{g} \vec{\xi}^\wedge$, see \eqref{eq:C3:xi}. Given the initial condition $\mat{g}(0,\cdot) = \mat{g}_0$, an approximation of the continuously deformable soft robot can be obtained by partial integration over the spatial domain$^{1}$: \blankfootnote{$^1$ The solution to \eqref{eq:C3:mappingSE3g} follows from the first-order ``\textit{Magnus Series Expansion}'' \cite{Renda2020}. Namely, \eqref{eq:C3:xi} can be written in the form $\YB' = \YB \AB(\sigma)$, and following \cite{Hairer2006} (see Theorem 7.1 from Magnus \cite{Magnus2006Oct}), it can solved explicitly by $\YB(\sigma) = \YB(0) \exp(\OmegaB(\sigma))$ with $\OmegaB$ the Magnus expansion of $\AB$. }
%
%\vspace{-2mm}
\begin{equation}
...
\end{align}
%
Given the fact that $\exp(-\ad_{\YB}) = \Ad\inv_{\exp(\hat{\YB})}$ for every $\YB$ belonging to the set $\seg{3}$ (see  Taylor \cite{Taylor2010Jan}, page 90), we can utilize the solution discovered in a manner similar to equation \eqref{eq:C3:mappingSE3g} to rephrase $\vec{\chi}(\sigma)$ as $\Ad\inv_{\gB(\sigma)}$. Consequently, assuming $\etaB = \vec{0}_6$, whereby the base of the soft robot is structurally fixed, we can derive a convenient expression for the approximation of the velocity twist:%$^1$\blankfootnote{$^1$ The derived solution is analogous to \cite{Renda2017Aug,Renda2018,Boyer2021}.}:
% Now, given the initial condition $\vec{\eta}(0,t) = \vec{0}_6$ and the approximations $[\vec{\xi}]_k$ and $[\mat{g}]_k$, and using the differential property of the adjoint action $-\mat{\ad}_{\vec{\xi}} = \p /\p \sigma [\Ad_{g^{-1}}] \Ad_{g}$ \cite{Murray1994}, we can solve the continuum forward kinematics in \eqref{eq:C3:pde_kin} as we can find an approximation to the velocity twist $\vec{\eta}$ by spatial integration:
%
\begin{align}
