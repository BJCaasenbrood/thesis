...
\end{equation}
%
To solve the above, let us introduce $\vec{\chi}(a) = \exp\big(-\ad_{\small \int_0^a\!\xiB(\chi)d\chi}\big)$, which allows us to conveniently represent the "\textit{state transition matrix}" as $\phiB(\sigma,s) = \exp\big(-\ad_{\small \int_s^\sigma\!\xiB(\chi)d\chi}\big) = \vec{\chi}(\sigma) \vec{\chi}\inv(s)$ \cite{Hairer2006,Magnus2006Oct}. Subsequently, fixing a time instance $t$, substituting the modal approximation of the strain rate $[\dot{\vec{\xi}}]_k = \PhiB \q$, and assuming the initial condition $\etaB_0 = \etaB(0,t)$, it is possible to obtain an analytical solution for equation \eqref{eq:eta_adg} in the following form:
%
\begin{align}
