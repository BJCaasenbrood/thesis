...
\end{figure*}
%
 We inspire our modified control law on the sensorimotor feedback control proposed by Wang et al. \cite{Wang2022Dec} (which itself is inspired from \cite{Justh2006Jun}) that is part of a series of seminal works on reproducing bio-inspired motion (primarily octopi) for Cosserat beam control \cite{Chang2022,Chang2023,Gazzola2018}. Rather than end-effector setpoint stabilization as in prior examples of this chapter, the works \cite{Wang2022Dec,Justh2006Jun} instead minimizes Euclidean distance between the target and any closest point on the backbone curve. Here, we propose several modifications: ($i$) we explore a energy-shaping technique \eqref{eq:C3:control} with full system information, $(ii)$ we introduce terms that also minimize the energy spent on elastic structural deformation via null-space projections \cite{Murray1994,Spong2006}, and $(iii)$ we use geodesics in $\SE{3}$ rather than in $\R^3$ like the Euclidean distance. The later implies we look at position as orientation information. The controller in \eqref{eq:C3:control} is modified by choosing a different desired potential energy function $\nabla_\q \,\mathcal{U}_d$:
%
\begin{equation}
...
\end{equation}
%
where $\JB_d := \JB(\sigma_d,\q)$, its pseudo inverse as $\JB_d^\dag = \mat{J}_d^\top\bigl(\JB_d {\JB_d}^\top + \lambda_2 \mat{I} \bigr)\inv$, $\lambda_1,\lambda_2,\lambda_3 > 0$ the control gains, and the optimal geometric error as $\hat{\ten{E}}_d = \log_{\textrm{SE}(3)}([\gB]_k\inv(\sigma_d,\q),\gB_d)$ with $\sigma_d$ to spatial coordinate that minimizes the geodesic distance between the backbone curve and the desired setpoint $\gB_d$. To be specific, this adaptive geodesic minimizer is found using
%
\begin{equation}
...
\end{equation}
%
which is found using a line search algorithm. 

We again consider the soft robot inspired by the octopus arm (see Table \ref{tab:C3:parameters1}) with the controller-observer gains: $\lambda_1 = \lambda_2 = 0.1$ and $\lambda_3 = 0.01$. The desired end-effector configuration is $\PhiB_d = \IB_3$ and $\gammaB_d = (70, 0, 40)^\top$. To simulate the viscous fluidic drag, we additionally add an external wrench $\ten{F}_{\textrm{drag}} = -\frac{1}{2}\rho_{\textrm{water}} \lVert\JB \dq\rVert \ten{A}_{\textrm{drag}} \JB \dq $ where $\rho_{\textrm{water}} = 1000$ $\text{kg}\text{m}^{-3}$ and $\ten{A}_{\textrm{drag}} \in \cose{e} \times \seg{3}$ a symmetric tensor related drag coefficient that are imposed by the cross-sectional geometry of the tentacle. This fluidic drag can then incorporated as an generalized force using $\tauB_{\textrm{drag}}= \int_\Xs \ten{F}_{\textrm{drag}} \; d\sigma$. 
...
\begin{figure}[!t]
  ...
  \vspace{-7mm}
  %\includegraphics*[width=\textwidth]{./pdf/thesis-figure-5-12.pdf}
  \caption{\small bla \label{fig:C3:bend_propogate}}
  %\vfill
\end{figure}
\clearpage
