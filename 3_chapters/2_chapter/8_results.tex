...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.99\textwidth]{fig_C2_setup_measure.pdf} 
  \caption{General overview of the optical vision system. \tcircle{1} Circular Hough transformation from the RBG camera data filtering for circles with a radius 32 \texttt{pix}, it has a global maxima at the optical marker. \tcircle{2} Sample of the stereo-vision depth camera. \tcircle{3} Final result of the sensor fusion algorithm of depth and RGB camera, resulting in an accurate estimate of soft manipulator's end-effector $\tvec{\gammaB}(L,\tvec{q})$. \label{fig:C2:setupmeasure}}
\end{figure}
\clearpage
}
...
\begin{example}[Model validation -- unforced, forced, and external loads] \end{example}
...
\begin{figure}[!t]
  ...
  dynamic model in unforced conditions, where the dashed lines represent the experimental measurements and the solid lines are the simulated trajectories. The dataset \data{Matlab1} are within the linear elastic regime whereas datasets (\ldata{Matlab2},\ldata{Matlab3}) are in the nonlinear regime. }
...
\begin{example}[Model benchmark -- Multi-link soft manipulator case]\end{example}
...
\begin{example}[Closed-loop -- Passivity-based controller under material certainty and mass disturbance]\end{example}
