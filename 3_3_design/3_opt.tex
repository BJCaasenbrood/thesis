...
\end{align}
%
\noindent where the Jacobian of the residual force vector is substituted by the tangent stiffness matrix $\KB_T:=\tfrac{\p \RB}{\p \x}$ (see \cite{Kim2018}). By choosing the adjoint vector $\boldsymbol{\lambda}^\top = \boldsymbol{L}^\top(\boldsymbol{K}_T)^{-1}$, the terms involving $\tfrac{\p \x}{\p \rho_e}$ can be eliminated and thus computation of the gradient becomes feasible. Following this, the gradient of the objective function can be written compactly as:
%
\begin{align}
